...
#if 0
    REGISTER_STRING_CONSTANT("BATTERIES_TABLE", "batteries",  CONST_CS|CONST_PERSISTENT);
    REGISTER_STRING_CONSTANT("CENTERS_TABLE",   "centers",  CONST_CS|CONST_PERSISTENT);
    REGISTER_STRING_CONSTANT("CHARGERS_TABLE",  "chargers",  CONST_CS|CONST_PERSISTENT);
    REGISTER_STRING_CONSTANT("PUMPS_TABLE",     "pumps",  CONST_CS|CONST_PERSISTENT);
    REGISTER_STRING_CONSTANT("INVERTERS_TABLE", "inverters",  CONST_CS|CONST_PERSISTENT);
    REGISTER_STRING_CONSTANT("WIRE_TABLE",      "wire",  CONST_CS|CONST_PERSISTENT);
    REGISTER_STRING_CONSTANT("LOADS_TABLE",     "loads",  CONST_CS|CONST_PERSISTENT);
    REGISTER_STRING_CONSTANT("VENDORS_TABLE",   "vendors",  CONST_CS|CONST_PERSISTENT);
    REGISTER_STRING_CONSTANT("PRICES_TABLE",    "prices",  CONST_CS|CONST_PERSISTENT);
    REGISTER_STRING_CONSTANT("MODULES_TABLE",   "modues",  CONST_CS|CONST_PERSISTENT);
    REGISTER_STRING_CONSTANT("COMBINERS_TABLE", "combiners",  CONST_CS|CONST_PERSISTENT);
    REGISTER_STRING_CONSTANT("PROJECTS_TABLE", "projects",  CONST_CS|CONST_PERSISTENT);
#endif
...
PHP_FUNCTION(gui_list_items)
{
    ...
    if (zend_parse_parameters(1 TSRMLS_CC, "l", &projid) == FAILURE) {
	WRONG_PARAM_COUNT;
    }

    item_t **items = (item_t **)gui_list_items(projid);
    zval *result = malloc(sizeof(zval));
    array_init(result);
//    GNUAE_G(items[GNUAE_G(count)]) = nitem;
